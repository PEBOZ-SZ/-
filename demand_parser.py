...
class DemandParseResult:
    ...
    raw_row_count: int = 0
    requirement_fields: dict[str, str] = field(default_factory=dict)
    materials_detail_rows: list[dict[str, str]] = field(default_factory=list)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "materials_detail_rows": self.materials_detail_rows,
...
def parse_demand_from_rows(
    ...
    *,
    ...
    requirement_fields = extract_requirement_fields_from_rows(rows)
    materials_detail_rows = extract_material_detail_rows_from_rows(rows)
    ...
    materials.extend(_extract_c_material_detail_materials(rows))
...
def _extract_c_material_detail_materials(rows: list[list[str]]) -> list[Material]:
    try:
        from admin_bom_requirement_view import extract_material_detail_rows_from_rows
    except Exception:
        return []
    materials: list[Material] = []
    for row in extract_material_detail_rows_from_rows(rows):
        name = str(row.get("standard_name_code") or "").strip()
        if not name or name == "无":
            continue
        role = str(row.get("type") or "").strip()
        spec = str(row.get("calculation_size") or "").strip()
        piece = str(row.get("piece_part") or "").strip()
        remark = str(row.get("remark") or "").strip()
        note_parts = [part for part in (piece, remark) if part and part != "无"]
        materials.append(
            Material(
                role=role if role and role != "无" else "C区材料明细",
                name=name,
                spec=spec if spec and spec != "无" else "",
                note=" / ".join(note_parts) if note_parts else "",
                source="c_material_detail",
            )
        )
    return materials
...
def _extract_implicit_quantity_section(rows: list[list[str]]) -> dict[str, str]:
    for idx, row in enumerate(rows):
        if not row_is_quantity_tier_header(row):
            continue
        if not _row_is_order_quantity_tier_header(row):
            continue
...
def _row_is_order_quantity_tier_header(row: list[str]) -> bool:
    keys = [str(normalize_parameter_key(str(cell or "")) or "").strip() for cell in row]
    keys = [key for key in keys if key]
    if not keys:
        return False
    qty_keys = [key for key in keys if re.fullmatch(r"(?:数量|qty)\d*", key, re.I)]
    if len(qty_keys) < 2:
        return False
    disallowed_prefixes = (
        "类型",
    ...
    if any(any(key.startswith(prefix) for prefix in disallowed_prefixes) for key in keys):
        return False
    allowed = re.compile(r"^(?:数量|qty|利润率|毛利率|利率)\d*$", re.I)
    return all(allowed.fullmatch(key) for key in keys)
...
def _row_is_material_detail_display_header(row: list[str]) -> bool:
    keys = {_normalise_key(cell) for cell in row if str(cell or "").strip()}
    if "类型" not in keys:
        return False
    has_name_code = bool(
        keys
    ...
    has_display_only_col = bool(
        keys & {"对应核算尺寸", "核算尺寸", "备注说明", "备注"}
        or any(_normalise_key(cell).startswith("部位/裁片") for cell in row if str(cell or "").strip())
    )
    return has_name_code and has_display_only_col
...
def _split_section_header_value_blocks(
    ...
    while idx < len(section_rows):
        ...
        if not _row_has_content(row):
            idx += 1
            continue
        if _row_is_material_detail_display_header(row):
            idx += 1
            while idx < len(section_rows):
...
def _inject_shoulder_strap_length_from_section_c(
    ...
    for m in materials:
        ...
        if piece_count_usage_from_cell_note(str(m.note or "")):
            continue
        if str(m.quoted_usage or "").strip():
            continue
        m.quoted_usage = lr
        return
...
def _is_canonical_quantity_tier_key(key: object) -> bool:
    norm_key = str(normalize_parameter_key(str(key or "")) or "").strip().lower()
    if norm_key.startswith("\u6570\u91cf"):
        return True
    return re.fullmatch(r"(?:quantity_?|qty)\d+", norm_key, re.I) is not None
    ...
    *,
    ...
    for src in sources:
        for key, value in src.items():
            if not _is_canonical_quantity_tier_key(key):
                continue
            qty = _extract_order_quantity_value(value)
            if qty is not None:
...
def _extract_order_quantity_value(value: object) -> int | None:
    text = str(value or "").strip()
    if not text:
        return None
    slash_tail = re.search(r"[/／]\s*(\d[\d,]*)", text)
    if slash_tail:
        return int(slash_tail.group(1).replace(",", ""))
    numbers = [int(m.group(0).replace(",", "")) for m in re.finditer(r"\d[\d,]*", text)]
    if not numbers:
        return None
    unit_pattern = re.compile(r"\d[\d,]*\s*(?:片|个|条|套|pcs?|PCS|只|粒)")
