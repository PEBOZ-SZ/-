...
_FABRIC_SPEC_RE = re.compile(
    r"(?:210|420|600|840|1000|1680)\s*[dD]|"
    r"(?:PU|PVC|牛津|尼龙|涤纶|塔丝隆|帆布|里布|外料|"
    r"oxford|nylon|polyester)",
    re.I,
)
_DIM_SPEC_RE = re.compile(
    r"\d+(?:\.\d+)?\s*(?:mm|cm|厘米|公分|米|m|码|yd)\b",
    re.I,
)
...
def _sanitize_image_inferred_pricing(row: dict[str, Any]) -> dict[str, Any]:
    """图片推断不得自动确定单价/用量/金额/面料规格/精确尺寸。"""
    out = dict(row)
    out["usage"] = "-"
    out["unit_price"] = "-"
    out["amount"] = 0.0
    ...
    if spec and (_FABRIC_SPEC_RE.search(spec) or _DIM_SPEC_RE.search(spec)):
        out["spec"] = "-"
    return out


def mark_image_inferred_row(row: dict[str, Any]) -> dict[str, Any]:
    out = _sanitize_image_inferred_pricing(dict(row))
    ...
    if PHOTO_INFERENCE_NOTE not in reason:
        reason = f"{reason}；{PHOTO_INFERENCE_NOTE}" if reason else PHOTO_INFERENCE_NOTE
