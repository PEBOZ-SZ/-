...
IMAGE_INFERENCE_NOTE = "图片推理，需人工复核"
...
def build_inferred_candidate_row(
    *,
    ...
    note = IMAGE_INFERENCE_NOTE if source_type == SOURCE_IMAGE else INFERENCE_NOTE
    row = {
        ...
        "recognition_reason": f"{label}推理：{snippet or component_name}；{note}",
        "calc_note": note,
...
def infer_missing_cost_candidates(
    ...
    *,
    ...
    allow_structure_inference = bool(inference_blob) and not demand_template
    if not allow_structure_inference and not (image_present and vision_text.strip()):
    ...
    for comp in (_all_detected_components(inference_blob) if allow_structure_inference else []):
