...
class NoQuotableItemsError(ValueError):
    """无可计价物料行，禁止回退默认演示材料。"""
...
def _default_line_items_for_mode(quote_mode: str) -> tuple[LineItem, ...]:
    items = default_line_items()
    if quote_mode != "demo_mode":
        return items
    return tuple(
        LineItem(
            name=item.name,
            ...
            source="default_demo",
            kb_hit=False,
            kb_auto_learned=item.kb_auto_learned,
            spec_ai=item.spec_ai,
            usage_ai=item.usage_ai,
            unit_price_ai=item.unit_price_ai,
            amount_ai=item.amount_ai,
            price_source=item.price_source,
            rule_applied=item.rule_applied,
            learning_rule_source=item.learning_rule_source,
            override_id=item.override_id,
            evidence=item.evidence,
            pricing_review_required=item.pricing_review_required,
            price_conflict_required=item.price_conflict_required,
            kb_matched_name=item.kb_matched_name,
            kb_matched_spec=item.kb_matched_spec,
            kb_reference_price=item.kb_reference_price,
            calc_note=item.calc_note,
            recognition_status=item.recognition_status,
            recognition_reason=item.recognition_reason,
            raw_usage=item.raw_usage,
            raw_unit_price=item.raw_unit_price,
            usage_unit_kind=item.usage_unit_kind,
            price_unit_kind=item.price_unit_kind,
            raw_quantity_unit=item.raw_quantity_unit,
            raw_price_unit=item.raw_price_unit,
            converted_quantity_unit=item.converted_quantity_unit,
            converted_price_unit=item.converted_price_unit,
            unit_converted=item.unit_converted,
            unit_conversion_basis=item.unit_conversion_basis,
        )
        for item in items
    )
...
def parse_order_quantities_from_quote_params(payload: dict[str, Any]) -> tuple[int, ...]:
    quote_params = payload.get("quote_params") if isinstance(payload, dict) else None
    fields = quote_params.get("F") if isinstance(quote_params, dict) else None
    if not isinstance(fields, dict):
        fields = payload.get("requirement_fields") if isinstance(payload, dict) else None
    if not isinstance(fields, dict):
        return ()
    values: list[int] = []
    for key, raw in fields.items():
        key_text = str(key or "").strip().lower().replace(" ", "")
        if not (key_text.startswith("数量") or key_text.startswith("quantity") or key_text.startswith("qty")):
            continue
        text = str(raw or "").replace(",", "").strip()
        if not text:
            continue
        slash = re.search(r"[/／]\s*(\d+)", text)
        if slash:
            qty = int(slash.group(1))
        else:
            nums = [int(x) for x in re.findall(r"\d+", text)]
            if not nums:
                continue
            qty = max(nums)
        if qty > 1 and qty not in values:
            values.append(qty)
    return tuple(sorted(values))


def resolve_payload_quantities(payload: dict[str, Any]) -> tuple[int, ...]:
    quantities = parse_quantities(payload.get("quantities"))
    order_quantities = parse_order_quantities_from_quote_params(payload)
    if order_quantities and (quantities == (1,) or 1 in quantities):
        payload["quantities"] = list(order_quantities)
...
def _allow_default_line_items(payload: dict[str, Any] | None) -> bool:
    if not isinstance(payload, dict):
        return True
    if parse_bool(payload.get("strict_quotable_items"), False):
        return False
    if parse_bool(payload.get("quote_confirmed"), False):
        return False
    return payload.get("items") is None
...
def parse_items(
    ...
    *,
    ...
    quote_mode: str = "draft_mode",
) -> tuple[LineItem, ...]:
    if not isinstance(value, list):
        return _default_line_items_for_mode(quote_mode) if allow_default_fallback else tuple()
    ...
    for row in value:
        ...
        confirmation_applied = parse_bool(row.get("quote_confirmation_applied"), False)
        if recalculated is not None:
            expected_amount, display_unit_price, calc_addition = recalculated
            if confirmation_applied or abs(expected_amount - amount_value) > 0.01:
                amount_value = expected_amount
                amount_ai = False if confirmation_applied else amount_ai
    ...
    return tuple(items) if items else (
        _default_line_items_for_mode(quote_mode) if allow_default_fallback else tuple()
    )
...
def annotate_detail_rows_quote_accuracy(
    ...
    for row in detail_rows:
        ...
        badge_hints = badge_accessory_unit_conflict_hints(r)
        ah = row_amount_crosscheck_hint(
            r.get("usage"),
            r.get("unit_price"),
            float(r.get("amount") or 0.0),
        )
        bag: list[str] = [*uh, *badge_hints]
        if ah:
            bag.append(ah)
            amt_rows += 1
        if uh:
...
def build_settings(payload: dict[str, Any] | None = None) -> QuoteSettings:
    ...
    quote_mode = normalize_quote_mode(payload.get("quote_mode"))
    allow_default = _allow_default_line_items(payload)
    ...
    return QuoteSettings(
        ...
        quantities=resolve_payload_quantities(payload),
        items=parse_items(
            payload.get("items"),
            structure_text=str(
            ...
            allow_default_fallback=allow_default,
            quote_mode=quote_mode,
        ),
    )


def _stamp_demo_default_sources(payload: dict[str, Any], quote_mode: str) -> None:
    if quote_mode != "demo_mode":
        return
    for key in (
        "mold_fee",
        ...
        payload.setdefault(f"{key}_source", "default_demo")
    if payload.get("items") is None:
        payload.setdefault("default_line_items", True)


def _blocked_quote_validation_response(validation: dict[str, Any]) -> dict[str, Any]:
    return {
        "quote_ready": False,
        "quote_mode": validation.get("quote_mode", "draft_mode"),
        "validation_status": "blocked",
        "validation_errors": list(validation.get("validation_errors") or []),
        "source_summary": dict(validation.get("source_summary") or {}),
    }


def _settings_source_summary(payload: dict[str, Any]) -> dict[str, str]:
    from quote_mode_validation import normalize_source
    ...
    for key in (
        ...
        source = normalize_source(payload.get(f"{key}_source"))
        if source:
            out[key] = source
    return out
...
def calculate_quote(payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    _stamp_demo_default_sources(payload, quote_mode)
    validation = validate_quote_payload_for_mode(payload, quote_mode=quote_mode)
    if validation.get("validation_status") == "blocked":
        return _blocked_quote_validation_response(validation)
    from material_spec_usage_enricher import (
    ...
    if not settings.items:
        raise NoQuotableItemsError("没有可参与报价的有效物料行，请补充单价/用量或勾选参与报价的材料。")
    ...
    result = {
        ...
        "quote_mode": validation.get("quote_mode", quote_mode),
        ...
        "settings_sources": _settings_source_summary(payload),
    ...
    result["source_summary"] = build_source_summary(payload, result=result)
