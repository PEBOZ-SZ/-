    ...
    if (result.quote_ready === false) {
      ...
      if (isQuoteConfirmationResult(result)) {
        ...
        state.pendingStructureConfirm = {
          ...
          confirmRowsSnap: confirmSnap.map((r) => ({ ...r })),
        ...
        replaceLoadingByToken(loadingToken, {
          ...
          data: state.pendingStructureConfirm.data,
          confirmToken: token,
          fileName: quoteFileLabel,
        });
...
function quoteConfirmFieldValue(field) {
  if (field && typeof field === "object") {
    const finalValue = field.final_value ?? field.manual_value ?? field.system_value ?? field.value;
    return finalValue == null ? "" : String(finalValue);
  }
  return field == null ? "" : String(field);
}

function isQuoteConfirmationResult(result) {
  if (!result || typeof result !== "object") {
    return false;
  }
  // 正式报价成功时后端仍会回传 quote_confirmation 快照，不能据此再次进入确认页
  if (result.quote_ready === true) {
    return false;
  }
  return (
...
function handleFinalQuoteConfirmationBlocked(pending, result, loadingToken) {
  if (result?.quote_confirmation && typeof result.quote_confirmation === "object") {
    pending.quoteConfirmation = result.quote_confirmation;
    pending.data = mergeStructureConfirmDataPreserveRows(pending.data, result);
  }
  replaceLoadingByToken(loadingToken, {
    role: "assistant",
    type: "text",
    text:
      ...
      result.message ||
      "仍有参与报价的材料缺少单价或用量，请在下方「结构/明细预览」补全并保存后重试。",
  });
  setComposerStatusLine(
    result.assistant_message || "请补全缺少的单价/用量后重试",
    "warn",
  );
}

function mapQuoteConfirmationMaterialsToStructureRows(materials) {
  ...
  return materials
    .map((row, idx) => {
      ...
      const name = quoteConfirmFieldValue(row.material_name) || `第${idx + 1}行`;
      const usage = quoteConfirmFieldValue(row.usage || row.quantity);
      const unitPrice = quoteConfirmFieldValue(row.unit_price);
      const unit = quoteConfirmFieldValue(row.measure_unit);
      const usageDisplay =
        usage && unit && !String(usage).includes(unit) ? `${usage}${unit}` : usage;
      ...
      const calcParts = [];
      if (row.included_in_quote === false) {
        calcParts.push("不参与报价");
      }
      if (notes) {
        calcParts.push(notes);
      }
      return {
        name,
        spec: quoteConfirmFieldValue(row.spec) || "",
        usage: usageDisplay,
        unit_price: unitPrice,
        calc_note: calcParts.join("；"),
        recognition_status,
        quote_row_id: row.row_id,
      };
    })
    .filter(Boolean);
}

function buildStructureConfirmRowsFromQuoteResult(result) {
  const fromItems =
    Array.isArray(result.items_confirmation) && result.items_confirmation.length > 0
      ? result.items_confirmation
          .map((r) => (r && typeof r === "object" ? { ...r } : {}))
          .filter((r) => !isReferenceOnlyRowName(r.name))
      : Array.isArray(result.items_preview)
        ? result.items_preview
  ...
  const confirmation =
    result.quote_confirmation && typeof result.quote_confirmation === "object"
      ? result.quote_confirmation
      : null;
  return mapQuoteConfirmationMaterialsToStructureRows(confirmation?.materials);
}

function mergeStructureConfirmDataPreserveRows(prevData, result) {
  const prev = prevData && typeof prevData === "object" ? prevData : {};
  const merged = { ...prev, ...result };
  const prevRows = getStructureConfirmationTableRows(prev);
  if (prevRows.length > 0) {
    if (Array.isArray(prev.items_confirmation) && prev.items_confirmation.length > 0) {
      merged.items_confirmation = prev.items_confirmation;
    }
    if (Array.isArray(prev.items_preview) && prev.items_preview.length > 0) {
      merged.items_preview = prev.items_preview;
    }
  } else {
    const mapped = buildStructureConfirmRowsFromQuoteResult(result);
    if (mapped.length > 0) {
      merged.items_preview = mapped;
    }
  }
  if (Array.isArray(prev.material_piece_summaries) && prev.material_piece_summaries.length > 0) {
    merged.material_piece_summaries = prev.material_piece_summaries;
  }
  if (prev.material_area_overview && typeof prev.material_area_overview === "object") {
    merged.material_area_overview = prev.material_area_overview;
  }
  if (prev.bom_requirement_view && typeof prev.bom_requirement_view === "object") {
    merged.bom_requirement_view = prev.bom_requirement_view;
  }
  return merged;
}

function enterQuoteConfirmModeFromResult(pending, result, loadingToken) {
  if (!pending) {
    return;
  }
  pending.quoteConfirmMode = true;
  pending.quoteConfirmation = result.quote_confirmation || pending.quoteConfirmation || null;
  pending.data = mergeStructureConfirmDataPreserveRows(pending.data, result);
  const mappedRows = getStructureConfirmationTableRows(pending.data);
  if (mappedRows.length > 0) {
    pending.confirmRowsSnap = mappedRows.map((r) => ({ ...r }));
  }
  replaceLoadingByToken(loadingToken, {
    role: "assistant",
    type: "structure_confirmation",
...
const MAT_DETAIL_MAIN_COL_COUNT = 6;
...
function formatAreaM2FromCm2(cm2) {
  ...
  return formatAreaM2Value(n / 10000);
}
...
function formatAreaM2Value(m2, cm2Fallback) {
  ...
  if (cm2Fallback != null) {
    return formatAreaM2FromCm2(cm2Fallback);
  }
  return "-";
}

function isValidStructureMaterialDetailRow(row) {
  if (!row || typeof row !== "object") {
    return false;
  }
...
function buildStructureMaterialDetailRows(data, pending) {
  ...
  if (brv && typeof brv === "object") {
    ...
    if (Array.isArray(direct) && direct.length) {
      return direct.filter(isValidStructureMaterialDetailRow);
    }
    const sections = Array.isArray(brv.sections) ? brv.sections : [];
    for (const section of sections) {
      ...
      const detailRows = section.detail_rows;
      if (Array.isArray(detailRows) && detailRows.length) {
        return detailRows.filter(isValidStructureMaterialDetailRow);
      }
    }
  }
  const summaries = Array.isArray(data?.material_piece_summaries) ? data.material_piece_summaries : [];
  if (summaries.length) {
    return summaries
      .map((summary, idx) => ({
        type: String(summary.material_type || `物料${idx + 1}`),
        standard_name_code: String(summary.material_name || ""),
        calculation_size: String(summary.calc_size_text || ""),
        remark: String(summary.summary_text || ""),
        source: String(summary.source || "C区材料明细"),
        _material_id: summary.material_id,
      }))
      .filter(isValidStructureMaterialDetailRow);
  }
  return getStructureConfirmationTableRows(data)
    .map((row, idx) => ({
      type: String(row.type || `物料${idx + 1}`),
      standard_name_code: String(row.name || ""),
      calculation_size: String(row.spec || ""),
      remark: String(row.calc_note || row.calc_method || ""),
      source: String(row.source_type || "BOM明细"),
    }))
    .filter(isValidStructureMaterialDetailRow);
}

function normalizeMaterialMatchKey(name, matType) {
  return `${String(matType || "").trim()}::${String(name || "").trim()}`.toLowerCase();
}

function buildMaterialSummaryLookup(data) {
  const summaries = Array.isArray(data?.material_piece_summaries) ? data.material_piece_summaries : [];
...
function lookupMaterialPieceSummary(lookup, row, index) {
  ...
  const name = String(row?.standard_name_code || row?.standard_name || row?.name || "").trim();
  const matType = String(row?.type || "").trim();
  const key = normalizeMaterialMatchKey(name, matType);
  const queue = lookup.byKeyQueues.get(key);
  if (queue && queue.length) {
    const consumed = lookup.consumedKeys.get(key) || 0;
    const hit = queue[consumed] || queue[0];
...
function materialDetailPreferredLossRate(summary) {
  const ds = summary?.display_summary;
  const rates = Array.isArray(ds?.loss_rates) ? ds.loss_rates.map(Number) : [3];
  return rates.find((rate) => Number.isFinite(rate) && rate > 0) ?? 3;
}

function materialDetailLossRateText(summary) {
  if (!summary) {
    return "-";
  }
  if (summary.is_area_measurable || summary.is_measurable) {
    return `${materialDetailPreferredLossRate(summary)}%`;
  }
  return "-";
}

function materialDetailTotalUsageText(summary, row) {
  if (summary) {
    const ds = summary.display_summary;
    const lossKey = String(materialDetailPreferredLossRate(summary));
    if (ds?.with_loss_m2_display?.[lossKey]) {
      return String(ds.with_loss_m2_display[lossKey]);
    }
    if (ds?.base_usage_m2_rounded && ds.base_usage_m2_rounded !== "—") {
      return String(ds.base_usage_m2_rounded);
    }
    if (summary.total_area_m2 != null || summary.total_area_cm2 != null) {
      return formatAreaM2Value(summary.total_area_m2, summary.total_area_cm2);
    }
...
function materialDetailSizeText(summary, row) {
  const calc = summary?.calc_size_text || row?.calculation_size || row?.calc_size || "";
  const text = String(calc).trim();
  return text && text !== "—" ? text : "-";
}

function materialDetailRiskBadges(summary) {
  if (!summary) {
    return "";
  }
  const badges = [];
  if (summary.size_conflict) {
    badges.push('<span class="mat-detail-badge mat-detail-badge--warn">尺寸待核</span>');
  }
  const hints = Array.isArray(summary.review_hints) ? summary.review_hints : [];
  if (hints.some((hint) => String(hint).includes("重复"))) {
    badges.push('<span class="mat-detail-badge mat-detail-badge--warn">可能重复</span>');
  }
  if (hints.some((hint) => /结构|纸样|备注/.test(String(hint)))) {
    badges.push('<span class="mat-detail-badge mat-detail-badge--info">需复核结构尺寸</span>');
  }
  const pending = Array.isArray(summary.pending_parts) ? summary.pending_parts : [];
  if (pending.length) {
    badges.push('<span class="mat-detail-badge mat-detail-badge--pending">待核</span>');
  }
  return badges.join("");
}

function renderMaterialPieceTableRowHtml(piece, index) {
...
function renderMaterialAreaExpandedBodyHtml(summary, row, pieces) {
  ...
  const covered =
    Array.isArray(summary?.covered_parts) && summary.covered_parts.length
      ...
      : "";
  const pending =
    Array.isArray(summary?.pending_parts) && summary.pending_parts.length
      ? `<p class="mat-area-detail-line mat-area-detail-warn"><strong>待复核：</strong>${escapeHtml(summary.pending_parts.join("、"))}</p>`
      : "";
  const excluded =
    Array.isArray(summary?.excluded_parts) && summary.excluded_parts.length
      ? `<p class="mat-area-detail-line"><strong>不含：</strong>${escapeHtml(summary.excluded_parts.join("、"))}</p>`
      : "";
  const sourceLine = `<p class="mat-area-detail-line"><strong>来源：</strong>${escapeHtml(String(summary?.source || row?.source || "C区材料明细"))}</p>`;
  const remarkText = String(row?.remark || "").trim();
  const remarkLine =
    remarkText && !["无", "-", "—"].includes(remarkText)
      ? `<div class="mat-area-detail-line"><strong>备注说明：</strong><div class="mat-area-remark">${escapeNl(remarkText)}</div></div>`
      : "";
  const calcSizeLine =
    summary?.calc_size_text && String(summary.calc_size_text).trim() && summary.calc_size_text !== "—"
      ? `<p class="mat-area-detail-line"><strong>对应核算尺寸（复核）：</strong>${escapeHtml(String(summary.calc_size_text))}</p>`
      : "";
  const structureSizeLine =
    summary?.structure_size_text &&
    String(summary.structure_size_text).trim() &&
    summary.structure_size_text !== "—"
      ? `<p class="mat-area-detail-line"><strong>备注解析结构尺寸：</strong>${escapeHtml(String(summary.structure_size_text))}</p>`
      : "";
  const hintLines = (Array.isArray(summary?.review_hints) ? summary.review_hints : [])
    .map((hint) => `<li>${escapeHtml(String(hint))}</li>`)
    .join("");
  const hintsBlock = hintLines
    ? `<ul class="mat-area-detail-hints">${hintLines}</ul>`
    : "";
  const summaryLine = summary?.summary_text
    ? `<p class="mat-area-detail-line"><strong>核算说明：</strong>${escapeHtml(String(summary.summary_text))}</p>`
    : "";
  const pieceRows = pieces.map((piece, idx) => renderMaterialPieceTableRowHtml(piece, idx)).join("");
  const pieceTable = pieceRows
...
function renderMaterialAreaBriefHtml(summary) {
  ...
  if (summary.total_area_m2 != null || summary.total_area_cm2 != null) {
    items.push(
      `<span class="mat-area-brief-item"><strong>汇总面积：</strong>${escapeHtml(formatAreaM2Value(summary.total_area_m2, summary.total_area_cm2))}</span>`,
    );
  }
  if (Array.isArray(summary.covered_parts) && summary.covered_parts.length) {
    items.push(
      `<span class="mat-area-brief-item"><strong>覆盖：</strong>${escapeHtml(summary.covered_parts.join("、"))}</span>`,
    );
  }
  if (Array.isArray(summary.pending_parts) && summary.pending_parts.length) {
    items.push(
      `<span class="mat-area-brief-item mat-area-brief-warn"><strong>待复核：</strong>${escapeHtml(summary.pending_parts.join("、"))}</span>`,
    );
  }
  const ds = summary.display_summary;
  const lossKey = String(materialDetailPreferredLossRate(summary));
  const lossDisp = ds?.with_loss_m2_display?.[lossKey];
  if (lossDisp) {
    items.push(`<span class="mat-area-brief-item"><strong>采购建议：</strong>${escapeHtml(String(lossDisp))}</span>`);
  }
  if (summary.size_conflict) {
    items.push('<span class="mat-area-brief-item mat-area-brief-warn"><strong>尺寸冲突</strong></span>');
  }
  if (!items.length && summary.summary_text) {
    items.push(`<span class="mat-area-brief-item">${escapeHtml(String(summary.summary_text))}</span>`);
  }
  return items.length ? `<div class="mat-area-brief">${items.join("")}</div>` : "";
}

function renderMaterialAreaDetailHtml(summary, row, token, idx) {
  const pieces = Array.isArray(summary?.pieces) ? summary.pieces.filter(Boolean) : [];
  const pieceCount = pieces.length;
  const brief = renderMaterialAreaBriefHtml(summary);
  const expanded = renderMaterialAreaExpandedBodyHtml(summary, row, pieces);
  const expandLabel = pieceCount > 0 ? `展开核算（含 ${pieceCount} 项裁片/部位）` : "展开核算";
  return `<div class="mat-detail-area" data-mat-detail-area="${escapeHtml(String(token || ""))}-${idx}">
    ${brief}
    <details class="mat-detail-details">
...
function renderMaterialDetailRowGroupHtml(row, summary, token, idx) {
  const matType = escapeHtml(String(row.type || "-"));
  const name = escapeHtml(String(row.standard_name_code || row.standard_name || "-"));
  ...
  const hasAreaDetail =
    summary &&
    ((Array.isArray(summary.pieces) && summary.pieces.length > 0) ||
      summary.summary_text ||
      (Array.isArray(summary.review_hints) && summary.review_hints.length > 0) ||
      summary.measure_summary);
  const mainRowClass = hasAreaDetail ? " mat-detail-has-area" : "";
  const main = `<tr class="mat-detail-main-row${mainRowClass}">
    <td>${matType}</td>
    <td class="mat-detail-name-cell"><span class="mat-detail-name">${name}</span>${badges}</td>
    <td>${size}</td>
    <td>${totalUsage}</td>
    <td>${qty}</td>
    <td>${loss}</td>
  ...
  const areaHtml = renderMaterialAreaDetailHtml(summary, row, token, idx);
  return `${main}<tr class="mat-detail-area-row"><td colspan="${MAT_DETAIL_MAIN_COL_COUNT}">${areaHtml}</td></tr>`;
}

function renderMaterialDetailTableSection(data, rows, token) {
  const lookup = buildMaterialSummaryLookup(data);
  const body = rows.map((row, idx) => renderMaterialDetailRowGroupHtml(row, lookupMaterialPieceSummary(lookup, row, idx), token, idx)).join("");
  const overview = data?.material_area_overview;
  let tfoot = "";
  if (overview && overview.total_before_dedup_m2 != null) {
    tfoot = `<tfoot><tr class="mat-detail-foot-row">
      <td colspan="${MAT_DETAIL_MAIN_COL_COUNT}">
        <span class="mat-detail-foot-label">整包基础面积合计（展示）：</span>
        <strong>${escapeHtml(formatAreaM2Value(overview.total_before_dedup_m2))}</strong>
        ${
          overview.procurement_hint_m2
            ? `<span class="muted"> · 含3%损耗参考 ${escapeHtml(String(overview.procurement_hint_m2))}㎡</span>`
            : ""
        }
      </td>
    </tr></tfoot>`;
  }
  return `<section class="structure-confirm-section structure-material-detail-section">
    <div class="structure-material-detail-shell">
      <div class="structure-confirm-section-head">
        <h4>材料明细</h4>
      </div>
      <p class="mat-detail-lead muted">面积核算已并入各行，点击「展开核算」可看裁片明细（仅展示，不改价）。</p>
      <div class="table-wrap mat-detail-table-wrap">
        <table class="mat-detail-table">
          <thead><tr>
            <th>类型</th><th>材料名</th><th>尺寸</th><th>总用量</th><th>数量</th><th>损耗</th>
          </tr></thead>
          <tbody>${body}</tbody>
          ${tfoot}
        </table>
...
function renderStructureMaterialDetailSection(data, token, pending) {
  ...
  return renderMaterialDetailTableSection(data, rows, token);
}
  ...
  try {
    ...
    if (result.quote_ready === false) {
      replaceLoadingByToken(loadingToken, {
        role: "assistant",
        type: "text",
        text: result.assistant_message || "报价输入已确认，但仍未生成报价，请检查表格数据。",
      });
      return;
    }
    ...
    syncPricingGateSnapshotFromQuote(result);
    setComposerStatusLine("正式报价已生成", "ok");
  } catch (error) {
    const message = humanizeQuoteFetchError(error instanceof Error ? error : new Error(String(error)));
    replaceLoadingByToken(loadingToken, {
      role: "assistant",
