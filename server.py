...
def _material_summaries_for_confirmation(payload: dict) -> list[dict]:
    ctx = dict(payload) if isinstance(payload, dict) else {}
    if not isinstance(ctx.get("bom_requirement_view"), dict):
        ctx["bom_requirement_view"] = build_admin_bom_requirement_view(payload, {})
    _enrich_quote_material_display(ctx)
    summaries = ctx.get("material_piece_summaries")
...
from material_row_dedupe import (
    ...
    merge_duplicate_quote_material_rows,
...
from sheet_kind_classifier import (
    KIND_UNKNOWN,
    build_sheet_kind_blocked_response,
    build_sheet_kind_unknown_response,
    classify_uploaded_sheet_kind,
...
def merge_structure_confirmation_user_items(
    ...
    for entry in patch_rows:
        ...
        if "remark" in entry:
            ...
            if len(remark) > max_str:
                remark = remark[:max_str]
            row["remark"] = remark
            touched = True
        section_raw = entry.get("section_key")
        if section_raw is None:
            section_raw = entry.get("area")
        if section_raw is not None:
            section_key = str(section_raw or "").strip().upper()
            if section_key:
                row["section_key"] = section_key
                row["area"] = section_key
                touched = True
        if "included_in_quote" in entry:
            included = bool(entry.get("included_in_quote"))
            row["included_in_quote"] = included
            row["amount_in_cost"] = included
            row["exclude_from_cost"] = not included
            touched = True
        elif "amount_in_cost" in entry or "exclude_from_cost" in entry:
            amount_in_cost = entry.get("amount_in_cost")
            exclude_from_cost = entry.get("exclude_from_cost")
            if amount_in_cost is not None:
                row["amount_in_cost"] = bool(amount_in_cost)
                touched = True
            if exclude_from_cost is not None:
                row["exclude_from_cost"] = bool(exclude_from_cost)
                touched = True
            if amount_in_cost is not None and exclude_from_cost is None:
                row["exclude_from_cost"] = not bool(amount_in_cost)
            elif exclude_from_cost is not None and amount_in_cost is None:
                row["amount_in_cost"] = not bool(exclude_from_cost)
        if touched:
    ...
    return promote_quotable_rows_for_quote(merge_duplicate_quote_material_rows(merged))
...
def _manual_requirement_quantities(fields: dict) -> list[int]:
    out: list[int] = []
    from demand_parser import _extract_order_quantity_value
    ...
    for key, raw in fields.items():
        if str(key) not in quantity_keys:
            continue
        qty = _extract_order_quantity_value(raw)
        if qty is not None and qty > 0 and qty not in out:
            out.append(qty)
    if out:
        return out
...
def _restore_uploaded_sheet_quantities_for_confirmed_quote(
    payload: dict,
    sheet_parse_result: dict | None,
) -> None:
    if not isinstance(payload, dict):
        return
    if not _parse_boolish(payload.get("quote_confirmed") or payload.get("quote_confirmed_by_user")):
        return
    quantity_sources = []
    quote_params = payload.get("quote_params")
    if isinstance(quote_params, dict):
        quantity_sources.append(quote_params.get("F"))
    quantity_sources.append(payload.get("requirement_fields"))
    if isinstance(sheet_parse_result, dict):
        quantity_sources.append(sheet_parse_result)

    quantities = []
    for source in quantity_sources:
        if not isinstance(source, dict):
            continue
        parsed = _manual_requirement_quantities(source)
        if not parsed and isinstance(source.get("quantities"), list):
            for raw in source.get("quantities") or []:
                try:
                    qty = int(float(str(raw).replace(",", "").strip()))
                except (TypeError, ValueError):
                    continue
                if qty > 0 and qty not in parsed:
                    parsed.append(qty)
        for qty in parsed:
    ...
    if not quantities:
        return
    existing = payload.get("quantities")
    if isinstance(existing, list) and existing and existing != [1]:
        return
    if quantities:
        payload["quantities"] = quantities


def _sync_confirmed_quote_quantities_to_response(payload: dict, response: dict) -> None:
    if not isinstance(payload, dict) or not isinstance(response, dict):
        return
    quantities = payload.get("quantities")
    tiers = response.get("tiers")
    if not (isinstance(quantities, list) and quantities):
        return
    if not (isinstance(tiers, list) and tiers):
        return
    first_qty = quantities[0]
    try:
        first_qty_int = int(first_qty)
    except (TypeError, ValueError):
        return
    first_tier = tiers[0] if isinstance(tiers[0], dict) else {}
    if first_tier.get("quantity") == first_qty_int:
        return
    response["quantity_mismatch_after_calc"] = {
        "payload_quantities": list(quantities),
        "tier0_quantity": first_tier.get("quantity"),
    }
...
def build_structure_confirmation_payload(
    ...
    *,
    ...
    if rows:
        ...
        rows = merge_duplicate_quote_material_rows(rows)
        ...
        confirmation_rows.append(
            {
                ...
                "merged_duplicate_count": r.get("merged_duplicate_count"),
                "merge_hint": str(r.get("merge_hint") or "").strip(),
                "source_row_indices": list(r.get("source_row_indices") or [])
                if isinstance(r.get("source_row_indices"), list)
                else [],
                "merged_from_rows": list(r.get("merged_from_rows") or [])
                if isinstance(r.get("merged_from_rows"), list)
                else [],
            },
        )
    ...
    if enrich_ctx.get("display_material_rows"):
        resp["display_material_rows"] = enrich_ctx["display_material_rows"]
...
def _prepare_quote_mode_for_calculation(payload: dict[str, Any]) -> str:
    from quote_mode_validation import normalize_quote_mode

    explicit = str(payload.get("quote_mode") or "").strip()
    strict_formal = str(os.environ.get("QUOTE_STRICT_FORMAL_VALIDATION") or "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
    ...
    if explicit:
        mode = normalize_quote_mode(explicit)
    elif strict_formal and bool(payload.get("quote_confirmed")):
        mode = "production_mode"
    elif bool(payload.get("demo_mode")) or bool(payload.get("default_line_items")):
        mode = "demo_mode"
    else:
        mode = "draft_mode"
    payload["quote_mode"] = mode
    return mode


def _stamp_confirmed_production_sources(payload: dict[str, Any]) -> None:
    if not isinstance(payload, dict) or not bool(payload.get("quote_confirmed")):
        return
    ...
    if isinstance(items, list):
        for row in items:
            if not isinstance(row, dict):
                continue
            included = (
                row.get("included_in_quote") is not False
                and row.get("amount_in_cost") is not False
                and row.get("exclude_from_cost") is not True
            ...
            for field in ("usage", "unit_price", "amount"):
                if row.get(field) not in (None, ""):
                    row[f"{field}_source"] = "user_input"
            ...
            row["confirmation_source"] = "structure_confirmed"
            for key in ("usage_ai", "unit_price_ai", "amount_ai", "pricing_review_required"):
                row.pop(key, None)
            if str(row.get("price_source") or "").strip() == "ai_estimate":
    ...
    for field in (
        ...
        if payload.get(field) not in (None, ""):
            payload.setdefault(f"{field}_source", "user_input")
    include_fob = payload.get("include_fob")
    include_fob_enabled = include_fob is not False and str(include_fob or "").strip().lower() not in {
        "0",
        "false",
        "no",
        "off",
    }
    if include_fob_enabled and payload.get("fob_addition") in (None, ""):
        payload["fob_addition"] = 4.0
        payload["fob_addition_source"] = "approved_rule"
    if payload.get("system_overhead") in (None, ""):
        payload["system_overhead"] = 4.0
        payload["system_overhead_source"] = "approved_rule"


def _quote_response_blocked_for_persistence(response: dict[str, Any]) -> bool:
    if not isinstance(response, dict):
        return False
    return (
        str(response.get("quote_mode") or "").strip() == "production_mode"
        and str(response.get("validation_status") or "").strip() == "blocked"
    )
...
def _resolve_front_sales_identity(handler: QuoteHandler) -> tuple[str, str, bool]:
    ...
    sid = str(ident.get("sales_user_id") or "").strip()
    if sid:
        name = str(ident.get("sales_user_name") or ident.get("sales_user_label") or "").strip()
        handler._cookie_sales_user_id = sid
        return sid, name, True
    return "", "", False


def _write_sales_identity_required(
    handler: QuoteHandler,
    *,
    message: str = "",
    sheet_hint: dict[str, Any] | None = None,
) -> None:
    handler.write_json(
        {
            "error": "sales_identity_required",
            "message": message or "未识别到业务员，请填写编号或姓名后继续报价。",
            "quote_ready": False,
            "sales_identity_required": True,
            "sheet_sales_hint": sheet_hint or {},
            "llm_status": get_kimi_status(),
        }
    )


def _bind_local_sales_identity_for_quote(
    handler: QuoteHandler,
    payload: dict[str, Any] | None,
) -> tuple[str, str] | None:
    """非企微：解析并绑定业务员；失败则写 sales_identity_required 响应。"""
    if wecom_enabled():
        return None
    ident = resolve_local_sales_identity(handler.headers.get("Cookie"), payload)
    sid = str(ident.get("sales_user_id") or "").strip()
    if not sid:
        hint: dict[str, Any] = {}
        if isinstance(payload, dict):
            fields = extract_sales_fields(payload.get("quote_params"))
            if fields.get("sales_code") or fields.get("sales_name"):
                hint = dict(fields)
        _write_sales_identity_required(handler, sheet_hint=hint)
        return None
    sname = str(ident.get("sales_user_name") or ident.get("sales_user_label") or "").strip()
    _set_front_sales_cookies(handler, sid, sname)
    handler._cookie_sales_user_id = sid
    if isinstance(payload, dict):
        payload["sales_user_id"] = sid
        payload["sales_user_name"] = sname
        if ident.get("sales_user_code"):
            payload["sales_user_code"] = ident["sales_user_code"]
        payload["identity_source"] = str(ident.get("identity_source") or "browser")
    return sid, sname
...
def _require_front_sales_auth(handler: QuoteHandler) -> tuple[str, str] | None:
    ...
    if wecom_enabled():
        ...
        return None
    _write_sales_identity_required(handler)
    return None
...
def _persist_quote_with_sales_user(
    ...
    *,
    ...
    if wecom_enabled():
        ...
        bound = _bind_local_sales_identity_for_quote(handler, payload)
        if bound is None:
            return False
        sales_uid, sales_name = bound
    ...
    finalize_quote_persistence(
        ...
        structured_input=copy.deepcopy(payload) if isinstance(payload, dict) else None,
        quote_mode=str(response.get("quote_mode") or (payload or {}).get("quote_mode") or ""),
        validation_status=str(response.get("validation_status") or ""),
        source_summary=response.get("source_summary") if isinstance(response.get("source_summary"), dict) else None,
    )
...
class QuoteHandler(BaseHTTPRequestHandler):
    ...
    def ensure_sales_user_id(self) -> str:
        """Local identity only (WECOM_ENABLED=0). Returns empty when unset."""
        if wecom_enabled():
            ...
            if verified:
                ...
                return verified[0]
            return ""
        ident = resolve_local_sales_identity(self.headers.get("Cookie"))
        sid = str(ident.get("sales_user_id") or "").strip()
        if sid:
            self._cookie_sales_user_id = sid
            return sid
        return ""
    ...
    def _admin_site_do_GET(self, parsed_req, req_path: str) -> None:
        ...
        if req_path == "/admin-api/quote-correction-learning/candidates":
            if not admin_http_access_ok(self.headers):
                self.write_json({"error": "forbidden", "message": "需要管理员权限。"}, status=403)
                return
            qs = parse_qs(parsed_req.query)
            status = (qs.get("status", ["pending"])[0] or "pending").strip()
            error_type = (qs.get("error_type", [""])[0] or "").strip()
            try:
                from quote_correction_learning import list_correction_candidates

                items = list_correction_candidates(status=status, error_type=error_type, limit=200)
            except Exception as exc:
                self.write_json({"error": "correction_learning_unavailable", "message": str(exc)}, status=500)
                return
            self.write_json({"items": items, "total": len(items)})
            return

        m_corr_cand = re.match(r"^/admin-api/quote-correction-learning/candidates/([^/]+)$", req_path)
        if m_corr_cand:
            if not admin_http_access_ok(self.headers):
                self.write_json({"error": "forbidden", "message": "需要管理员权限。"}, status=403)
                return
            cid = m_corr_cand.group(1)
            try:
                from quote_correction_learning import get_correction_candidate

                item = get_correction_candidate(cid)
            except Exception as exc:
                self.write_json({"error": "correction_learning_unavailable", "message": str(exc)}, status=500)
                return
            if not item:
                self.write_json({"error": "not_found", "message": "候选不存在。"}, status=404)
                return
            self.write_json({"item": item})
            return

        if req_path == "/admin-api/quote-correction-learning/rules":
            if not admin_http_access_ok(self.headers):
                self.write_json({"error": "forbidden", "message": "需要管理员权限。"}, status=403)
                return
            qs = parse_qs(parsed_req.query)
            ...
            if enabled_raw in {"1", "true", "yes"}:
                ...
                enabled = False
            try:
                from quote_correction_learning import list_correction_rules

                items = list_correction_rules(status=status, enabled=enabled, limit=200)
            except Exception as exc:
                self.write_json({"error": "correction_learning_unavailable", "message": str(exc)}, status=500)
                return
            self.write_json({"items": items, "total": len(items)})
        ...
        if req_path.startswith("/admin"):
            ...
            if req_path == "/admin/prices":
                ...
                return
            if req_path == "/admin/correction-learning":
                self.serve_static(STATIC_DIR / "admin" / "correction-learning.html")
                return
            self.send_response(302)
    ...
    def handle_quote_client_actions(self, payload: dict) -> bool:
        ...
        merged["quantities"] = [n]
        merged["quote_mode"] = "production_mode"
        response = calculate_quote(merged)
        if _quote_response_blocked_for_persistence(response):
            response["quote_ready"] = False
            self.write_json(response, status=400)
            return True
    ...
    def do_GET(self) -> None:
        ...
        if self.path == "/api/auth/status" or self.path.startswith("/api/auth/status?"):
            ...
            ident = resolve_local_sales_identity(self.headers.get("Cookie")) if not wecom_enabled() else {}
            self.write_json(
                auth_status_payload(
                    sales_user_id=sales_uid,
                    sales_user_name=sales_name,
                    authenticated=ok and bool(sales_uid),
                    identity_source=str(ident.get("identity_source") or ""),
                )
            )
            return
    ...
    def _admin_site_do_POST(self) -> None:
        ...
        m_corr_approve = re.match(
            r"^/admin-api/quote-correction-learning/candidates/([^/]+)/approve/?$",
            path,
        )
        if m_corr_approve:
            if not admin_http_access_ok(self.headers):
            ...
            if not isinstance(body, dict):
                body = {}
            try:
                from quote_correction_learning import approve_correction_candidate

                result = approve_correction_candidate(
                    m_corr_approve.group(1),
                    reviewed_by=str(body.get("reviewed_by") or body.get("operator") or "admin"),
                    review_note=str(body.get("review_note") or body.get("note") or ""),
                )
            except ValueError as exc:
                self.write_json({"error": "invalid_request", "message": str(exc)}, status=400)
                return
            except Exception as exc:
                self.write_json({"error": "correction_approve_failed", "message": str(exc)}, status=500)
                return
            self.write_json(result)
            return

        m_corr_reject = re.match(
            r"^/admin-api/quote-correction-learning/candidates/([^/]+)/reject/?$",
            path,
        )
        if m_corr_reject:
            if not admin_http_access_ok(self.headers):
            ...
            if not isinstance(body, dict):
                body = {}
            try:
                from quote_correction_learning import reject_correction_candidate

                result = reject_correction_candidate(
                    m_corr_reject.group(1),
                    reviewed_by=str(body.get("reviewed_by") or body.get("operator") or "admin"),
                    review_note=str(body.get("review_note") or body.get("note") or ""),
                )
            except ValueError as exc:
                ...
                self.write_json({"error": "correction_reject_failed", "message": str(exc)}, status=500)
                return
            self.write_json(result)
            return

        m_corr_toggle = re.match(
            r"^/admin-api/quote-correction-learning/rules/([^/]+)/toggle/?$",
            path,
        )
        if m_corr_toggle:
            if not admin_http_access_ok(self.headers):
            ...
            try:
                from quote_correction_learning import toggle_correction_rule

                result = toggle_correction_rule(
                    m_corr_toggle.group(1),
                    bool(enabled),
                    operator=str(body.get("operator") or body.get("reviewed_by") or "admin"),
                )
            except ValueError as exc:
                self.write_json({"error": "invalid_request", "message": str(exc)}, status=400)
                return
            except Exception as exc:
                self.write_json({"error": "correction_toggle_failed", "message": str(exc)}, status=500)
                return
            self.write_json(result)
            return
    ...
    def do_POST(self) -> None:
        ...
        if path_only == "/api/auth/sales-identity":
            if wecom_enabled():
                self.write_json(
                    {"error": "wecom_enabled", "message": "企业微信模式下请使用 OAuth 登录。"},
                    status=404,
                )
                return
            body = self.read_json()
            if not isinstance(body, dict):
                ...
                return
            raw_input = str(body.get("sales_rep_input") or "").strip()
            if raw_input:
                ident = normalize_sales_rep(raw_input, identity_source="manual")
            else:
                ident = build_sales_identity(
                    str(body.get("sales_user_code") or "").strip(),
                    str(body.get("sales_user_name") or "").strip(),
                    identity_source="manual",
                )
            if not ident.get("sales_user_id"):
                self.write_json(
                    {
                        "error": "invalid_sales_rep",
                        "message": "请填写有效的业务员编号或姓名。",
                    },
                    status=400,
                )
                return
            _set_front_sales_cookies(
                self,
                ident["sales_user_id"],
                ident.get("sales_user_name") or ident.get("sales_user_label") or "",
            )
            self.write_json(
                {
                    "ok": True,
                    ...
                    **auth_status_payload(
                        ...
                        sales_user_name=ident.get("sales_user_name") or "",
                        authenticated=True,
                        identity_source=str(ident.get("identity_source") or "manual"),
                    ),
                }
            )
            return
        ...
        if self.path in ("/api/quote", "/api/quotes/save"):
            ...
            if self.handle_session_intent_quote(payload):
                return
            uploaded_sheet_obj = payload.get("uploaded_sheet") if isinstance(payload, dict) else None
            if (
                isinstance(uploaded_sheet_obj, dict)
                and not structure_confirmed_early
                and not should_force_customer_demand_sheet(payload, user_text=str(payload.get("user_prompt") or payload.get("prompt") or ""))
            ):
                sheet_kind, sheet_kind_detail = classify_uploaded_sheet_kind(
                    uploaded_sheet_obj,
                    user_text=str(payload.get("user_prompt") or payload.get("prompt") or ""),
                )
                payload["upload_sheet_kind"] = sheet_kind
                if is_blocked_sheet_kind(sheet_kind):
                    blocked = build_sheet_kind_blocked_response(sheet_kind, sheet_kind_detail)
                    blocked["llm_status"] = get_kimi_status()
                    self.write_json(blocked)
                    return
                if sheet_kind == KIND_UNKNOWN:
                    unknown = build_sheet_kind_unknown_response(sheet_kind_detail)
                    unknown["llm_status"] = get_kimi_status()
                    self.write_json(unknown)
                    return
                payload["upload_sheet_kind_detail"] = sheet_kind_detail
            ...
            if not wecom_enabled():
                bound = _bind_local_sales_identity_for_quote(self, payload)
                if bound is None:
                    return
            ...
            if structure_confirmed_early:
                apply_manual_requirement_overrides_to_payload(payload)
                _restore_uploaded_sheet_quantities_for_confirmed_quote(payload, sheet_parse_result)
            ...
            if structure_confirmed and isinstance(payload.get("items"), list):
                ...
                from badge_unit_guard import apply_badge_unit_guard

                payload["items"] = apply_badge_unit_guard(list(payload.get("items") or []))
            ...
            if isinstance(payload.get("items"), list):
                ...
                payload["items"] = merge_duplicate_quote_material_rows(list(payload["items"]))
            from quote_confirmation import (
                apply_quote_confirmation_to_items,
                build_quote_confirmation,
                build_quote_confirmation_payload,
                merge_quote_confirmation_overrides,
                should_require_quote_confirmation,
                validate_quote_confirmation_for_calc,
            )

            if should_require_quote_confirmation(payload):
                summaries = _material_summaries_for_confirmation(payload)
                resp = build_quote_confirmation_payload(payload, material_summaries=summaries)
                resp["llm_status"] = llm_status
                resp["llm_audit"] = build_llm_audit(llm_audit_collector, llm_status)
                if sheet_parse_result is not None:
                    resp["sheet_parse"] = sheet_parse_result
            ...
            if bool(payload.get("quote_confirmed")):
                summaries = _material_summaries_for_confirmation(payload)
                confirmation = build_quote_confirmation(payload, material_summaries=summaries)
                if isinstance(payload.get("quote_confirmation"), dict):
                    confirmation = merge_quote_confirmation_overrides(
                        confirmation,
                        payload["quote_confirmation"],
                    )
                ok_confirm, confirm_msg, confirmation = validate_quote_confirmation_for_calc(
                    confirmation
                )
                payload["quote_confirmation"] = confirmation
                if not ok_confirm:
                    resp = build_quote_confirmation_payload(
                        payload,
                        material_summaries=summaries,
                    )
                    resp["quote_confirmation"] = confirmation
                    resp["assistant_message"] = confirm_msg
                    resp["confirmation_risks"] = list(resp.get("confirmation_risks") or []) + [
                        confirm_msg
                    ]
                    resp["error"] = "quote_confirmation_incomplete"
                    resp["llm_status"] = llm_status
                    if sheet_parse_result is not None:
                        resp["sheet_parse"] = sheet_parse_result
                    self.write_json(resp, status=400)
                    return
                payload["items"] = apply_quote_confirmation_to_items(
                    list(payload.get("items") or []),
                    confirmation,
                )
                payload["items"] = merge_duplicate_quote_material_rows(list(payload.get("items") or []))
                payload["strict_quotable_items"] = True
                _stamp_confirmed_production_sources(payload)
            _log_quote_stage(
                self,
                "pre_calculate",
            ...
            _stamp_confirmed_production_sources(payload)
            _restore_uploaded_sheet_quantities_for_confirmed_quote(payload, sheet_parse_result)
            _prepare_quote_mode_for_calculation(payload)
            try:
                ...
                from quote_engine import NoQuotableItemsError

                if isinstance(exc, NoQuotableItemsError):
                    self.write_json(
                        {
                            "error": "no_quotable_items",
                            "message": str(exc),
                            "quote_ready": False,
                            "assistant_message": str(exc),
                            "quote_confirmation": payload.get("quote_confirmation"),
                            "llm_status": llm_status,
                        },
                        status=400,
                    )
            ...
            if _quote_response_blocked_for_persistence(response):
                response["quote_ready"] = False
                response.setdefault("error", "quote_validation_blocked")
                response.setdefault("message", "正式报价校验未通过。")
                response["llm_status"] = llm_status
                if sheet_parse_result is not None:
            ...
            response["quote_id"] = str(uuid.uuid4())
            if isinstance(payload.get("quote_confirmation"), dict):
                response["quote_confirmation"] = payload["quote_confirmation"]
            response["llm_status"] = llm_status
    ...
    def try_parse_demand_template(self, payload: dict) -> DemandParseResult | None:
        ...
        if not parsed.sections:
            return None
        if not parsed.materials and not parsed.materials_detail_rows:
            return None
