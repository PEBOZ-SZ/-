...
from message_intent import looks_like_follow_up, looks_like_quote_explain_follow_up, should_explain_quote_without_requote
...
def route_quote_request(
    ...
    *,
    ...
    if has_active_quote and _looks_like_alternatives_request(text) and not _has_negotiate_signal(text):
        return _direct_or_clarify(ROUTE_QUOTE_PATCH, 0.86, "active_quote_alternatives")
    ...
    if has_active_quote and _looks_like_process_breakdown(text):
        return _direct_or_clarify(ROUTE_EXPLAIN, 0.88, "active_quote_process_breakdown")
...
def _looks_like_alternatives_request(text: str) -> bool:
    return bool(
        re.search(
            r"(替代方案|替代料|两个替代|2个替代|两个方案|备选方案|给我两个)",
            str(text or ""),
            re.I,
        )
    )
...
def _looks_like_process_breakdown(text: str) -> bool:
    if not str(text or "").strip():
        return False
    if not re.search(
        r"(计算过程|过程拆解|成本构成|公式是怎样的|逐行|明细怎么来)",
        str(text),
        re.I,
    ):
        return False
    return not should_explain_quote_without_requote(str(text))
