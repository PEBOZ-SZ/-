...
_DDL_FRAGMENTS = [
    ...
    "ALTER TABLE quote_versions ADD COLUMN IF NOT EXISTS structured_input_json TEXT",
    "ALTER TABLE quote_versions ADD COLUMN IF NOT EXISTS quote_mode TEXT",
    "ALTER TABLE quote_versions ADD COLUMN IF NOT EXISTS validation_status TEXT",
    "ALTER TABLE quote_versions ADD COLUMN IF NOT EXISTS base_version_no INTEGER",
    "ALTER TABLE quote_versions ADD COLUMN IF NOT EXISTS base_calc_quote_id TEXT",
    "ALTER TABLE quote_versions ADD COLUMN IF NOT EXISTS patch_id TEXT",
    "ALTER TABLE quote_versions ADD COLUMN IF NOT EXISTS source_summary_json TEXT",
    """
    ...
    CREATE TABLE IF NOT EXISTS quote_sessions (
        ...
        calc_quote_id TEXT NOT NULL,
        version_no INTEGER NOT NULL,
        created_at TEXT NOT NULL,
        updated_at TEXT NOT NULL,
        metadata_json TEXT,
        PRIMARY KEY (session_id, quote_series_uid, calc_quote_id)
    )
    """,
    "CREATE INDEX IF NOT EXISTS idx_quote_sessions_series ON quote_sessions(quote_series_uid, version_no DESC)",
    """
    CREATE TABLE IF NOT EXISTS quote_patches (
        patch_id TEXT PRIMARY KEY,
        quote_series_uid TEXT NOT NULL,
        base_calc_quote_id TEXT,
        base_version_no INTEGER,
        new_calc_quote_id TEXT,
    ...
    """,
    "CREATE INDEX IF NOT EXISTS idx_quote_patches_series ON quote_patches(quote_series_uid, created_at DESC)",
    """
    ...
    """,
    "CREATE INDEX IF NOT EXISTS idx_quote_audit_logs_series ON quote_audit_logs(quote_series_uid, created_at DESC)",
    """
...
def save_quote_calculation(
    *,
    ...
    base_calc_quote_id: str | None = None,
    ...
    structured_obj = (
        ...
        else quote_result.get("structured_input")
        if isinstance(quote_result.get("structured_input"), dict)
        else None
    )
    source_summary_obj = (
        source_summary
        if isinstance(source_summary, dict)
        else quote_result.get("source_summary")
        if isinstance(quote_result.get("source_summary"), dict)
        else None
    )
    structured_input_json = _json_dumps_or_none(structured_obj)
    source_summary_json = _json_dumps_or_none(source_summary_obj)
    quote_mode_str = str(quote_mode or quote_result.get("quote_mode") or "").strip() or None
    validation_status_str = (
        str(validation_status or quote_result.get("validation_status") or "").strip() or None
    )
    base_ver_val = _coerce_optional_int(
        base_version_no if base_version_no is not None else quote_result.get("base_version_no")
    )
    base_calc_id_str = str(base_calc_quote_id or quote_result.get("base_calc_quote_id") or "").strip() or None
    patch_id_str = str(patch_id or quote_result.get("patch_id") or "").strip() or None
        ...
        with _connect_ctx() as conn:
            try:
                ...
                conn.execute(
                    ...
                    INSERT INTO quote_versions (
                        quote_uid, version_no, calc_quote_id, saved_at, intent, quote_json,
                        structured_input_json, quote_mode, validation_status,
                        base_version_no, base_calc_quote_id, patch_id, source_summary_json
                    ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                    """,
                    (
                        q_uid,
...
def load_quote_version_structured_input(quote_uid: str, version_no: int) -> dict[str, Any] | None:
    from psycopg.rows import dict_row

    q_uid = str(quote_uid or "").strip()
    ver = _coerce_optional_int(version_no)
    if not q_uid or ver is None or ver <= 0:
        return None
    init_quote_storage()
    with _PG_LOCK:
        with _connect_ctx() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute(
                    """
                    SELECT structured_input_json FROM quote_versions
                    WHERE quote_uid = %s AND version_no = %s
                    LIMIT 1
                    """,
                    (q_uid, ver),
    ...
    if not row:
        return None
    obj = _json_loads_dict(row.get("structured_input_json"))
    return obj or None


def resolve_quote_version_target(
    quote_uid: str,
    *,
    version_no: int | None = None,
    calc_quote_id: str | None = None,
    ...
    with _PG_LOCK:
        with _connect_ctx() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                if calc_quote_id:
                    cur.execute(
                        "SELECT * FROM quote_versions WHERE quote_uid = %s AND calc_quote_id = %s LIMIT 1",
                        (q_uid, str(calc_quote_id)),
                    )
                elif version_no is not None:
                    cur.execute(
                        "SELECT * FROM quote_versions WHERE quote_uid = %s AND version_no = %s LIMIT 1",
    ...
    if not row:
        return None
    out = dict(row)
    out["structured_input"] = _json_loads_dict(out.get("structured_input_json"))
    out["source_summary"] = _json_loads_dict(out.get("source_summary_json"))
    return out


def save_quote_patch(
    *,
    patch_id: str | None = None,
    quote_series_uid: str,
    base_calc_quote_id: str | None = None,
    base_version_no: int | None = None,
    new_calc_quote_id: str | None = None,
    new_version_no: int | None = None,
    patch_type: str = "",
    patch_json: dict[str, Any] | None = None,
    status: str = "trial_preview",
    created_by: str | None = None,
) -> dict[str, Any]:
    init_quote_storage()
    pid = str(patch_id or uuid.uuid4().hex).strip()
    ts = utc_now_iso()
    patch_blob = _json_dumps_or_none(patch_json if isinstance(patch_json, dict) else {}) or "{}"
    row = {
        "patch_id": pid,
        "quote_series_uid": str(quote_series_uid or "").strip(),
        "base_calc_quote_id": str(base_calc_quote_id or "").strip() or None,
        "base_version_no": _coerce_optional_int(base_version_no),
        "new_calc_quote_id": str(new_calc_quote_id or "").strip() or None,
        "new_version_no": _coerce_optional_int(new_version_no),
        "patch_type": str(patch_type or "").strip(),
        "patch_json": patch_json if isinstance(patch_json, dict) else {},
        "status": str(status or "trial_preview").strip() or "trial_preview",
    ...
    with _PG_LOCK:
        with _connect_ctx() as conn:
            conn.execute(
                """
                INSERT INTO quote_patches (
                    patch_id, quote_series_uid, base_calc_quote_id, base_version_no,
                    new_calc_quote_id, new_version_no, patch_type, patch_json,
                    status, created_by, created_at
                ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                ON CONFLICT (patch_id) DO UPDATE SET
                    quote_series_uid = EXCLUDED.quote_series_uid,
                    base_calc_quote_id = EXCLUDED.base_calc_quote_id,
                    base_version_no = EXCLUDED.base_version_no,
                    new_calc_quote_id = EXCLUDED.new_calc_quote_id,
                    new_version_no = EXCLUDED.new_version_no,
                ...
                (
                    row["patch_id"],
                    row["quote_series_uid"],
                    row["base_calc_quote_id"],
                    row["base_version_no"],
                    row["new_calc_quote_id"],
                    row["new_version_no"],
                    row["patch_type"],
                    patch_blob,
                    row["status"],
...
def save_quote_audit_log(
    *,
    ...
    init_quote_storage()
    aid = str(audit_id or uuid.uuid4().hex).strip()
    ts = utc_now_iso()
    payload_blob = _json_dumps_or_none(event_payload if isinstance(event_payload, dict) else {}) or "{}"
    row = {
        "audit_id": aid,
        "quote_series_uid": str(quote_series_uid or "").strip(),
        "calc_quote_id": str(calc_quote_id or "").strip() or None,
        "version_no": _coerce_optional_int(version_no),
        ...
        with _connect_ctx() as conn:
            conn.execute(
                ...
                INSERT INTO quote_audit_logs (
                    audit_id, quote_series_uid, calc_quote_id, version_no,
...
def list_quote_chat_messages(quote_series_uid: str, *, limit: int = 500) -> list[dict[str, Any]]:
    ...
    with _PG_LOCK:
        with _connect_ctx() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                ...
                rows = cur.fetchall()
    out = [_chat_message_dict_from_row(dict(row)) for row in rows]

    def _has_frontend_persist_index(row: dict[str, Any]) -> bool:
        meta = row.get("metadata") if isinstance(row.get("metadata"), dict) else {}
        return meta.get("persist_index") is not None

    def _is_legacy_auto_message(row: dict[str, Any], quote_ids: set[str]) -> bool:
        mid = str(row.get("message_id") or "")
        meta = row.get("metadata") if isinstance(row.get("metadata"), dict) else {}
        msg_type = str(meta.get("type") or "").strip()
        if msg_type == "quote_card" and any(mid == f"qc-{qid}" for qid in quote_ids):
            return True
        if msg_type == "user_turn" and any(mid == f"user-{qid}-turn" for qid in quote_ids):
            return True
        return False

    if any(_has_frontend_persist_index(row) for row in out):
        frontend_quote_ids = {q_uid}
        for row in out:
            meta = row.get("metadata") if isinstance(row.get("metadata"), dict) else {}
            if _has_frontend_persist_index(row) and str(meta.get("type") or "") == "quote_card":
                qid = str(meta.get("quote_id") or "").strip()
                if qid:
                    frontend_quote_ids.add(qid)
        out = [row for row in out if not _is_legacy_auto_message(row, frontend_quote_ids)]

    def _message_sort_key(row: dict[str, Any]) -> tuple[str, int, int, str]:
        meta = row.get("metadata") if isinstance(row.get("metadata"), dict) else {}
        try:
            persist_index = int(meta.get("persist_index"))
        except (TypeError, ValueError):
            persist_index = 10**9
        role_order = {"user": 0, "assistant": 1, "admin": 2, "system": 3}.get(
    ...
    return sorted(out, key=_message_sort_key)[:lim]
