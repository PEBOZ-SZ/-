...
def ensure_correction_tables(conn: sqlite3.Connection) -> None:
    ...
    for name, ddl in {
        ...
        "product_type_pattern": "TEXT NOT NULL DEFAULT ''",
        "product_name_pattern": "TEXT NOT NULL DEFAULT ''",
        "material_spec_pattern": "TEXT NOT NULL DEFAULT ''",
        "size_condition_json": "TEXT NOT NULL DEFAULT ''",
        "structure_keywords": "TEXT NOT NULL DEFAULT '[]'",
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS quote_correction_candidates (
            ...
            field_name TEXT,
            system_usage TEXT,
            system_unit_price TEXT,
            system_amount REAL,
            corrected_usage TEXT,
            corrected_unit_price TEXT,
            corrected_amount REAL,
            system_total_cost REAL,
            corrected_total_cost REAL,
            total_cost_gap REAL,
            total_cost_gap_pct REAL,
            error_type TEXT,
            confidence REAL,
            reason TEXT,
            evidence_json TEXT,
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_qcc_status ON quote_correction_candidates(status, created_at DESC)"
    )
...
def _rule_matches_row(rule: CorrectionRule, row: dict[str, Any], ctx: dict[str, Any]) -> bool:
    ...
    if rule.match_product_keywords and not _matches_keywords(pn, rule.match_product_keywords):
        return False
    try:
        from quote_correction_candidate_pool import rule_row_extended_match

        if rule.rule_id.startswith(LEARNED_RULE_ID_PREFIX):
            ext_ctx = {
                **ctx,
                "material_name": name,
                "material_spec": str(row.get("spec") or ctx.get("material_spec") or ""),
                "spec": str(row.get("spec") or ""),
                "product_type": str(ctx.get("product_type") or ""),
            }
            conn = _connect()
            try:
                db_row = conn.execute(
                    "SELECT * FROM quote_correction_rules WHERE rule_id = ?",
                    (rule.rule_id,),
                ).fetchone()
            finally:
                if _TEST_CONN is None:
                    conn.close()
            if db_row and not rule_row_extended_match(db_row, ext_ctx):
                return False
    except Exception:
        pass
    return True
...
def _admin_approved_usage_bad_value_override(row: dict[str, Any], rule: CorrectionRule) -> bool:
    """已审批 learning 规则：bad_values 命中当前错误用量时可覆盖显式 BOM 行。"""
    if not rule.rule_id.startswith(LEARNED_RULE_ID_PREFIX):
        return False
    if not rule.bad_values:
        return False
    cur = _current_field_value(row, "usage")
    if not cur or is_missing_spec_usage_value(cur):
        return False
    return _value_matches_bad(cur, rule.bad_values)
...
def _may_apply_usage_rule(row: dict[str, Any], rule: CorrectionRule) -> bool:
    ...
    explicit_override = bool(
        is_explicit_bom_usage_row(row) and _admin_approved_usage_bad_value_override(row, rule)
    )
    if is_explicit_bom_usage_row(row) and not explicit_override:
        return False
    if explicit_override:
        return bool(rule.corrected_value) and not is_dynamic_rule_usage_token(rule.corrected_value)
...
def _apply_usage_rule(
    ...
    explicit_bom_override = bool(
        is_explicit_bom_usage_row(row) and _admin_approved_usage_bad_value_override(row, rule)
    )
    mode = "applied" if rule.confidence >= APPLY_CONFIDENCE_MIN else "suggested"
    if mode == "applied":
        row["correction_before_usage"] = old_val
        row["correction_before_unit_price"] = str(row.get("unit_price") or "")
        try:
            row["correction_before_amount"] = float(row.get("amount") or 0)
        except (TypeError, ValueError):
            row["correction_before_amount"] = 0.0
        row["usage"] = rule.corrected_value
        row["usage_ai"] = False
        if explicit_bom_override:
            for key in RAW_USAGE_KEYS:
                row[key] = rule.corrected_value
            row["_bom_usage_locked"] = True
        if rule.affects_calculation:
            row["correction_rule_id"] = rule.rule_id
            row["correction_rule_source"] = "quote_correction_rules"
            row["correction_rule_applied"] = True
            row["correction_reason"] = rule.reason or rule.rule_type
...
def apply_quote_applicable_rules_to_payload(payload: dict[str, Any]) -> list[RuleApplication]:
    ...
    ctx = {
        ...
        or (payload.get("sheet_metadata") or {}).get("产品类型")
        or "",
        "product_size": payload.get("product_size") if isinstance(payload.get("product_size"), dict) else {},
    }
...
def capture_bom_edit_corrections(
    ...
    *,
    ...
    try:
        from quote_correction_candidate_pool import capture_correction_candidates_from_bom_save
        ...
        if cids:
            result.warnings.append(f"已生成 {len(cids)} 条待审核纠错候选")
    except Exception:
        logger.debug("capture_correction_candidates_from_bom_save skipped", exc_info=True)
    return result
...
# --- 候选池 / 审核 API（复用 quote_correction_candidate_pool） ---
def classify_correction_error(old_row, new_row, context=None):
    from quote_correction_candidate_pool import classify_correction_error as _fn

    return _fn(old_row, new_row, context)


def list_correction_candidates(**kwargs):
    from quote_correction_candidate_pool import list_correction_candidates as _fn

    return _fn(**kwargs)


def get_correction_candidate(candidate_id):
    from quote_correction_candidate_pool import get_correction_candidate as _fn

    return _fn(candidate_id)


def approve_correction_candidate(candidate_id, **kwargs):
    from quote_correction_candidate_pool import approve_correction_candidate as _fn

    return _fn(candidate_id, **kwargs)


def reject_correction_candidate(candidate_id, **kwargs):
    from quote_correction_candidate_pool import reject_correction_candidate as _fn

    return _fn(candidate_id, **kwargs)


def list_correction_rules(**kwargs):
    from quote_correction_candidate_pool import list_correction_rules as _fn

    return _fn(**kwargs)


def toggle_correction_rule(rule_id, enabled, **kwargs):
    from quote_correction_candidate_pool import toggle_correction_rule as _fn

    return _fn(rule_id, enabled, **kwargs)


def build_rule_from_candidate(candidate):
    from quote_correction_candidate_pool import build_rule_from_candidate as _fn

    return _fn(candidate)
