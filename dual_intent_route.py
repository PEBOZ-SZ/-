...
def infer_dual_route(resp: Mapping[str, Any], *, http_status: int) -> tuple[str, float, str]:
    ...
    if reply_type == "quote_confirmation":
        return DUAL_INTENT_CLARIFY, 0.85, "quote_confirmation_gate"
...
def apply_dual_mode_envelope(resp: dict[str, Any], *, trace: Mapping[str, Any], http_status: int) -> None:
    ...
    legacy_flow = resp.get("intent")
    legacy_str = str(legacy_flow or "").strip() if legacy_flow is not None else ""

    reply_type = str(resp.get("reply_type") or "").strip().lower()
    resp["dual_intent"] = dual_intent

    if legacy_str and legacy_str not in DUAL_INTENTS:
        resp["intent"] = legacy_str
        resp["flow_intent"] = legacy_str
    elif reply_type == "structure_confirmation":
        resp["intent"] = "STRUCTURE_CONFIRMATION_REQUIRED"
        resp["flow_intent"] = "STRUCTURE_CONFIRMATION_REQUIRED"
    elif reply_type == "quote_confirmation":
        resp["intent"] = "QUOTE_CONFIRMATION_REQUIRED"
        resp["flow_intent"] = "QUOTE_CONFIRMATION_REQUIRED"
    else:
