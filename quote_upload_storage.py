...
def _ensure_quote_versions_columns(conn: sqlite3.Connection) -> None:
    cols = {row[1] for row in conn.execute("PRAGMA table_info(quote_versions)").fetchall()}
    additions = {
        "structured_input_json": "TEXT",
        "quote_mode": "TEXT",
    ...
    for name, ddl in additions.items():
        if name not in cols:
            conn.execute(f"ALTER TABLE quote_versions ADD COLUMN {name} {ddl}")
...
def init_quote_storage() -> None:
    ...
    with _DB_LOCK:
        ...
        try:
            ...
            _ensure_quote_versions_columns(conn)
...
def _is_deleted_quote_row(row: object) -> bool:
...
def _filter_deleted_quote_rows(rows: object) -> list[dict[str, Any]]:
    if not isinstance(rows, list):
        return []
    return [r for r in rows if isinstance(r, dict) and not _is_deleted_quote_row(r)]
    ...
    *,
    ...
    if configured_quote_db_backend() == "postgres":
        ...
        postgres_impl.save_quote_calculation(
            ...
            base_calc_quote_id=base_calc_quote_id,
            patch_id=patch_id,
    ...
    if isinstance(quote_result.get("detail_rows"), list):
        quote_result["detail_rows"] = _filter_deleted_quote_rows(quote_result.get("detail_rows"))
...
def load_quote_version_object(quote_uid: str, version_no: int) -> dict[str, Any] | None:
    ...
    if configured_quote_db_backend() == "postgres":
        ...
        return postgres_impl.load_quote_version_object(q_uid, ver)
    init_quote_storage()
    with _DB_LOCK:
        conn = _connect()
        try:
            return _load_quote_version_object(conn, q_uid, ver)
        finally:
            conn.close()
...
def load_quote_version_structured_input(quote_uid: str, version_no: int) -> dict[str, Any] | None:
    ...
    if configured_quote_db_backend() == "postgres":
        ...
        return postgres_impl.load_quote_version_structured_input(q_uid, ver)
    init_quote_storage()
    with _DB_LOCK:
        conn = _connect()
        try:
    ...
    if not row:
        return None
    obj = _json_loads_dict(row[0] if not isinstance(row, sqlite3.Row) else row["structured_input_json"])
    return obj or None
...
def resolve_quote_version_target(
    ...
    *,
    ...
    if configured_quote_db_backend() == "postgres":
        ...
        return postgres_impl.resolve_quote_version_target(
            q_uid,
    ...
    with _DB_LOCK:
        ...
        try:
            params: tuple[Any, ...]
            if calc_quote_id:
                sql = "SELECT * FROM quote_versions WHERE quote_uid = ? AND calc_quote_id = ? LIMIT 1"
                params = (q_uid, str(calc_quote_id))
            elif version_no is not None:
                sql = "SELECT * FROM quote_versions WHERE quote_uid = ? AND version_no = ? LIMIT 1"
                params = (q_uid, int(version_no))
            else:
                sql = """
            ...
            conn.close()
    return _quote_version_row_to_dict(row)
...
def save_quote_patch(
    *,
    ...
    if configured_quote_db_backend() == "postgres":
        ...
        return postgres_impl.save_quote_patch(
            patch_id=patch_id,
            quote_series_uid=quote_series_uid,
            base_calc_quote_id=base_calc_quote_id,
            base_version_no=base_version_no,
            new_calc_quote_id=new_calc_quote_id,
    ...
    with _DB_LOCK:
        ...
        try:
            conn.execute(
                """
                INSERT OR REPLACE INTO quote_patches (
...
def save_quote_audit_log(
    *,
    ...
    if configured_quote_db_backend() == "postgres":
        ...
        return postgres_impl.save_quote_audit_log(
            quote_series_uid=quote_series_uid,
            calc_quote_id=calc_quote_id,
            version_no=version_no,
...
def _quote_version_row_to_dict(row: sqlite3.Row | None) -> dict[str, Any] | None:
    if not row:
        return None
    out = dict(row)
...
def _get_saved_quote_admin_bundle_sqlite(
    ...
    quote_obj["bom_requirement_view"] = build_admin_bom_requirement_view(quote_obj, meta_out)
    from material_piece_summary import enrich_quote_material_piece_summaries

    enrich_quote_material_piece_summaries(quote_obj)
    return {
        "meta": meta_out,
        "quote": quote_obj,
        "items": items_db,
