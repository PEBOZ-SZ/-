...
def auth_status_payload(
    *,
    ...
    src = str(identity_source or "").strip()
    if enabled and authenticated:
        src = "wecom"
    elif not enabled and authenticated and not src:
        src = "local"
    label = ""
    if authenticated:
        label = sales_display_name(sales_user_id, explicit_name=sales_user_name)
    return {
        ...
        "sales_user_label": label if authenticated else "",
        "login_url": login_url,
        "auto_login": bool(enabled and cfg is not None and not authenticated),
        "identity_source": src,
        "sales_identity_required": bool(not enabled and not authenticated),
