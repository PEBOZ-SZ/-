...
def _row_unit_conflict_hints(row: dict[str, Any], payload_row: dict[str, Any] | None = None) -> list[str]:
    ...
    badge_hints = badge_accessory_unit_conflict_hints(payload_row if isinstance(payload_row, dict) else row)
    if badge_hints:
        return badge_hints
    ...
    for idx, row in enumerate(rows):
        ...
        if hints:
            ...
            if usage_missing_main:
                ...
                r["validation_status"] = "HIGH_RISK"
                r["unit_price"] = "待补价"
                r["price_match_status"] = "未匹配单价"
                r["validation_detail"] = "核心物料单价缺失，未匹配单价，需补价后才能生成最终报价"
            elif calc_bad:
