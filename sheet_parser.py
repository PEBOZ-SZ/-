...
def material_detail_rows_to_items(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    buckets: dict[tuple[str, str], dict[str, Any]] = {}
    order: list[tuple[str, str]] = []
    for raw in rows:
        if not isinstance(raw, dict):
            continue
        name = str(raw.get("standard_name_code") or "").strip()
        role = str(raw.get("type") or "").strip()
        calc_size = str(raw.get("calculation_size") or "").strip()
        piece_part = str(raw.get("piece_part") or "").strip()
        remark = str(raw.get("remark") or "").strip()
        if not name or name == "无":
            continue
        if all(
            str(raw.get(key) or "").strip() in {"", "无"}
            for key in ("type", "standard_name_code", "calculation_size", "piece_part", "remark")
        ):
            continue
        calc_parts = [part for part in (piece_part, remark) if part and part != "无"]
        section_key = str(raw.get("pricing_section") or "C")
        key = (name.lower().replace(" ", ""), section_key.lower().replace(" ", ""))
        if key not in buckets:
            order.append(key)
            buckets[key] = {
                "name": name,
                "role": role if role and role != "无" else "C区材料明细",
                "sizes": [],
                "notes": [],
                "parts": [],
                "section_key": section_key,
                "included_in_quote": str(raw.get("included_in_quote") or "是") != "否",
            }
        bucket = buckets[key]
        if calc_size and calc_size != "无" and calc_size not in bucket["sizes"]:
            bucket["sizes"].append(calc_size)
        note = " / ".join(calc_parts) if calc_parts else ""
        if note and note not in bucket["notes"]:
            bucket["notes"].append(note)
        if piece_part and piece_part != "无" and piece_part not in bucket["parts"]:
            bucket["parts"].append(piece_part)
        bucket["included_in_quote"] = bool(bucket["included_in_quote"]) or str(raw.get("included_in_quote") or "是") != "否"
    items: list[dict[str, Any]] = []
    for key in order:
        bucket = buckets[key]
        sizes = bucket["sizes"]
        item = {
            "name": bucket["name"],
            "role": bucket["role"],
            "spec": sizes[0] if len(sizes) == 1 else ("、".join(sizes) if sizes else "-"),
            "usage": "-",
            "unit_price": "-",
            "amount": 0.0,
            "calc_note": "；".join(bucket["notes"]),
            "piece_part": "、".join(bucket["parts"]),
            "section_key": bucket["section_key"],
            "included_in_quote": bucket["included_in_quote"],
...
def parse_sheet_items_from_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    from admin_bom_requirement_view import (
        ...
        has_horizontal_c_material_detail_layout,
    )

    requirement_fields = extract_requirement_fields_from_rows(parsed_sheet.rows)
    materials_detail_rows = extract_material_detail_rows_from_rows(parsed_sheet.rows)
    requirement_fields = enrich_requirement_fields_from_material_details(
        requirement_fields,
        materials_detail_rows,
    )
    material_detail_items = material_detail_rows_to_items(materials_detail_rows)
    horizontal_c_layout = has_horizontal_c_material_detail_layout(materials_detail_rows)
    if material_detail_items and horizontal_c_layout:
        merged_items = material_detail_items
    elif material_detail_items:
        merged_items = merge_extracted_items(material_detail_items, merged_items)
    ...
    return {
        ...
        "materials_detail_rows": materials_detail_rows,
        "items": merged_items,
    }
...
DEMAND_SHEET_NAME_KEYWORDS = ("需求表", "填写区", "业务报价")
NON_DATA_SHEET_NAME_KEYWORDS = ("下拉选项", "字段映射", "使用说明", "填写说明", "说明", "选项")


def demand_sheet_name_score(name: str) -> int:
    text = str(name or "").strip().lower()
    return sum(1 for keyword in DEMAND_SHEET_NAME_KEYWORDS if keyword.lower() in text)


def non_data_sheet_name_penalty(name: str) -> int:
    text = str(name or "").strip().lower()
    return sum(1 for keyword in NON_DATA_SHEET_NAME_KEYWORDS if keyword.lower() in text)


def count_titled_template_sections(rows: list[list[str]]) -> int:
    seen: set[str] = set()
    for row in rows[:80]:
        ...
        if letter in {"A", "B", "C", "D", "E", "F", "G"} and title:
            seen.add(letter)
    return len(seen)


def choose_best_sheet(parsed_sheets: list[ParsedSheet]) -> ParsedSheet:
    def score_sheet(sheet: ParsedSheet) -> tuple[int, int, int]:
        extraction = rows_to_items(sheet.rows, start_row=None)
        keyword_bonus = 20 if contains_material_keyword(sheet.sheet_name) else 0
        name_score = demand_sheet_name_score(sheet.sheet_name)
        section_count = count_titled_template_sections(sheet.rows)
        demand_score = name_score * 1000
        if name_score > 0 or section_count >= 3:
            demand_score += section_count * 200
        demand_score -= non_data_sheet_name_penalty(sheet.sheet_name) * 500
        non_empty_rows = sum(1 for row in sheet.rows if any(cell for cell in row))
        item_score = len(extraction.items) * 10 + keyword_bonus
        return (demand_score, item_score, non_empty_rows)
