...
def admin_recalc_and_save_bom(
    ...
    *,
    ...
    payload["quote_mode"] = "production_mode"
    calc = calculate_quote(payload)
    if str(calc.get("validation_status") or "") == "blocked":
        return {
            "ok": False,
            "error": "quote_validation_blocked",
            "message": "正式报价校验未通过。",
            "validation_errors": list(calc.get("validation_errors") or []),
            "quote": calc,
        }
    ...
    base_version_no = int(meta.get("selected_version_no") or meta.get("latest_version_no") or 0) or None
    base_calc_quote_id = str(meta.get("selected_calc_quote_id") or meta.get("latest_calc_quote_id") or "").strip() or None
    ...
    save_quote_calculation(
        ...
        quote_mode=str(calc.get("quote_mode") or payload.get("quote_mode") or ""),
        validation_status=str(calc.get("validation_status") or ""),
        base_version_no=base_version_no,
        base_calc_quote_id=base_calc_quote_id,
        source_summary=calc.get("source_summary") if isinstance(calc.get("source_summary"), dict) else None,
    )
    save_quote_audit_log(
        quote_series_uid=quote_uid,
        calc_quote_id=calc_id,
        version_no=(base_version_no + 1) if base_version_no else None,
        event_type="admin_bom_recalc",
        event_payload={
            "base_version_no": base_version_no,
            "base_calc_quote_id": base_calc_quote_id,
            "calc_quote_id": calc_id,
